...
* A Redshift cluster inside the VPC and spanned across 2 Public Subnets selected. This reference deployment places the cluster in public subnets so that Kinesis Firehose and QuickSight can reach it without additional VPC networking. Note that this makes the cluster internet-facing; the only control limiting access is the security group below, which restricts ingress to the Firehose and QuickSight CIDR ranges. For a more secure deployment you can place the cluster in private subnets and provide private connectivity (NAT / VPC endpoints) for Firehose and QuickSight instead.
...
* Encryption Key inside KMS to encrypt data within Firehose and S3. The Redshift cluster is always encrypted at rest: when the "EncryptData" parameter is set to "yes" it uses this customer-managed KMS key, otherwise it falls back to the default AWS-managed key.

Below screen shots captures the parameters used by the CloudFormation, while deploying this stack inside AWS console. 

Each parameter has a description related to its usage in the architecture. You will have to select the VPC and Subnets where you want to deploy the Redshift cluster. If you deployed the VPC using the Step 1, then this will be the VPC and Subnet from the previous step. The Redshift cluster is always encrypted at rest; the "EncryptData" parameter only controls whether a customer-managed KMS key is used (set it to "yes") or the default AWS-managed key (the default, "no"). Most of the other parameters have been defaulted and can be modified, as per your needs. One parameter that needs to be provided is the Password for your Redshift Cluster. Make sure to note down the "Master user name" and "Master user password", as you would need this to access your Cluster.
